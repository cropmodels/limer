limer <- function(method, units = "meq/100g", ...){
  ...
  if(meth == "my"){
    ...
    lime <- my_lime(l$TAS, l$exch_ac, l$ECEC, a = l$a, b = l$b, clay = l$clay)
  }
  
  
  if(meth == "ka"){
    message("using Kamprath (1970) method")
    lime <- ka_lime(l$exch_ac)
  }
  ...
  if(meth == "co"){
    ...
    lime <- co_lime(l$TAS, l$exch_ac, exch_Ca, exch_Mg)
  }
  ...
  if(meth == "nu"){
    ...
    lime <- nu_lime(l$TAS, l$exch_ac, l$ECEC, l$clay)
  }
  ...
  if(meth %in% c("bv","br")){
    ...
    lime <- bv_lime(exch_Ca, exch_Mg, exch_K, exch_Na, 
  ...
  if(meth %in% c("te", "gt")){
    ...
    lime <- gt_lime(l$pH, l$OM, l$pot_ac, l$X, l$exch_Ca, l$exch_Mg)
  }
...
# Kamprath (1970) 
ka_lime <- function(exch_Al, lf = 1.5){
  lime <- exch_Al * lf
  return(lime)
}
...
# Cochrane, et al. (1980).
co_lime <- function(TAS, exch_Al, exch_Ca, exch_Mg){
...
nu_lime <- function(TAS, exch_ac, ECEC, clay){
...
my_lime <- function(TAS, exch_ac, ECEC, a = 0.8, b = 0.2, clay = NULL){
  if(!is.null(clay)){
...
gt_lime <- function(pH, OM, pot_ac, X = NULL, exch_Ca = NULL, exch_Mg = NULL){

...
# after that you only have the maintenance cost
# initial and maintenance in some currency unit
# interest rate is a fraction (0.1 = 10%)
...
annualized_cost <- function(initial, maintenance, interest_rate, years) {
    # subtract maintenance such that payment is same in all years.
    initial <- pmax(0, initial - maintenance)
    # only compute for the first year, as the total amount will be the same
    # for all years even if the balance between principal and interest changes
    interest_pay <- initial * interest_rate
    # interest rate must be > 0; but not worth it to check for that here
    # interest_rate <- max(interest_rate, 0.0000000001)

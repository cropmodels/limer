...
The data used by Cochrane et al. is available in `data(coch)`. Below I calculate recommended lime rates with `limeRate(method = "co")` to compare them to the actual lime rates and the ones calculated by them, as reported in their study. 
...
coch$pLR_meq <- limeRate(coch[, c("exch_Al","ECEC")], method = "co", 
...
coch$nu_LR <- limeRate(coch[, c("exch_Al","ECEC", "clay")], method = "nu", TAS = coch$Al_sat, check_Ca = FALSE)
